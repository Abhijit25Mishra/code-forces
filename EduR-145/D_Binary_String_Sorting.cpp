...
void solve()
{
    ...
    deque<char> x;
    fr(s.length()) { x.pb(s[i]); }

    while (true)
    {
        if (x.empty()) { cout << 0; nl; return; }
        else if (x.front() == '0') { x.pop_front(); }
        else if (x.back() == '1') { x.pop_back(); }
        else { break; }
    }
    ...
    // ll start = ans[0];
    sort(all(ab), sortbyCond);
    ll m = ab.size() / 2;
    ll ansval = 0;
    fr(m)
    {
        ansval += ab[i].first;
        if (ab[i].second == 0) { ok = false; }
    }
    ansval *= 1000000000001;
    if (ok)
    {
        ansval--;
    }
